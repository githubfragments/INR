...
experiment_names = [i.split('/')[-4] for i in
                    glob.glob(exp_root + "/KODAK21_*nerf*reg1*/kodim21/checkpoints/model_best_.pth")]
# experiment_names = [i.split('/')[-4] for i in
#                     glob.glob(exp_root + "/KODAK21_*nerf*noise/kodim21/checkpoints/model_best_.pth")]
#experiment_names = [i.split('/')[-4] for i in
...
for bw in [8]: #[5, 6, 7, 8]:
    ...
    for experiment_name in experiment_names:
        ...
        for im in imglob:
           ...
           # with open(os.path.join(exp_folder, 'result_best.json'), 'w') as fp:
            ...
            #bw=6
            lr=0.000001
            model_quantized, metrics, bytes, state_dict = quantize_model(model, bitwidth=bw, retrain=True, ref_model=ref_model, adaround=True, lr=lr)#, flags=TRAINING_FLAGS)
            model.load_state_dict(state_dict, strict=True)
            m = check_metrics(dataloader, model, image_resolution)
            print(m)
    ...
    # plt.legend()
    out_dict = df.to_dict()
    run_name='mlp_train_nerf_l1reg1e-05_symmetric_adaround_{}bit_500iter_lr{}_1000epochs'.format(bw,lr)
    #run_name='noise_test'
