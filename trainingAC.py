...
from utils import check_metrics_full
        ...
        for epoch in range(epochs):
            ...
            for step, (model_input, gt) in enumerate(train_dataloader):
                ...
                with torch.cuda.amp.autocast(enabled=use_amp):
                    ...
                    if not total_steps % steps_til_summary:
                        tqdm.write("Epoch %d, Total loss %0.6f, iteration time %0.6f, best mse %0.6f" % (epoch, train_loss, time.time() - start_time,  best_mse))
...
def train_quant_noise(model, train_dataloader, epochs, lr, steps_til_summary, epochs_til_checkpoint, model_dir, loss_fn,
          summary_fn, val_dataloader=None, double_precision=False, clip_grad=False, use_lbfgs=False, loss_schedules=None,
          weight_decay=0, l1_reg=0,  l1_loss_fn=model_l1,  spec_reg=0):
 ...
 # set_to_none=True here can modestly improve performance
    ...
    with tqdm(total=len(train_dataloader) * epochs) as pbar:
        ...
        for epoch in range(epochs):
            ...
            #     bitwidth=11
            #     sigma = torch.std(param)
            #     step_size = 2 * sigma / (2**bitwidth)
            #     noise = torch.rand_like(param) * 0.5 * step_size #add uniform noise of half the stepsize
            #     add_noise(param, noise)
